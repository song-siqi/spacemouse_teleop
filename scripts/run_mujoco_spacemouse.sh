...
SHOW_COLLISION_GEOMS="${SPACEMOUSE_TELEOP_SHOW_COLLISION_GEOMS:-0}"
...
MAX_EE_ANGULAR_SPEED="${SPACEMOUSE_TELEOP_MAX_EE_ANGULAR_SPEED:-0.25}"
...
ARGS=(
  ...
  --max-ee-angular-speed "$MAX_EE_ANGULAR_SPEED"
...
if [[ "$WANT_VIEWER" != "0" ]]; then
  ...
  if [[ "$SHOW_COLLISION_GEOMS" != "0" ]]; then
    ARGS+=(--show-collision-geoms)
  fi
